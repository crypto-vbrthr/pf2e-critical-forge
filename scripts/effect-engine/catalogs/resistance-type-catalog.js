...
function groupDefinition(id) {
  const labels = {
    "damage-types": "PF2E_CRITICAL_FORGE.EffectForge.ResistanceTypeGroups.DamageTypes",
    "damage-categories": "PF2E_CRITICAL_FORGE.EffectForge.ResistanceTypeGroups.DamageCategories",
    sources: "PF2E_CRITICAL_FORGE.EffectForge.ResistanceTypeGroups.Sources",
    additional: "PF2E_CRITICAL_FORGE.EffectForge.ResistanceTypeGroups.Additional"
  };

  return { id, label: localize(labels[id]) };
}

function configuredResistanceTypes() {
  const configured = CONFIG.PF2E?.resistanceTypes ?? {};
  ...
  // Standard damage types are valid resistance types and keep the catalog
  // useful during init, tests, and compatible PF2e releases with partial CONFIG.
  ...
  for (const [value, labelName] of Object.entries(FALLBACK_LABEL_KEYS)) {
    ...
    definitions.set(value, {
      value,
      label: localize(
        `PF2E_CRITICAL_FORGE.EffectForge.ResistanceTypeLabels.${labelName}`
      ),
      source: "fallback"
    });
  }
...
export function listResistanceTypeDefinitions() {
  return [...configuredResistanceTypes().values()]
    .map((definition) => {
...
export function getResistanceTypeDefinition(value) {
  const normalized = String(value ?? "").trim();
  return listResistanceTypeDefinitions().find((entry) => entry.value === normalized) ?? null;
}

export function isKnownResistanceType(value) {
  return Boolean(getResistanceTypeDefinition(value));
}

export function getResistanceTypeGroups(selected = null) {
  const selectedValue = String(selected ?? "").trim();
  const grouped = new Map();

  for (const definition of listResistanceTypeDefinitions()) {
